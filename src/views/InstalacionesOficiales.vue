<template>
  <div class="container">
    <h1>Instalaciones oficiales accesibles</h1>
    <h3>
      Para acceder a la información y ubicación de las
      <b>instalaciones accesibles de carácter oficial</b> en Madrid, selecciona
    ...
    <div v-if="informacion.datos" class="row infoInstitucion">
      ...
      {{ componente }}
      <div v-if="componente" class="col-12 col-md-6">
...
export default {
  ...
  setup() {
    ...
    let componente = ref(false);

    function verInstalaciones() {
      fetch(
        `https://datos.madrid.es/egob/catalogo/202162-0-instalaciones-accesibles-municip.json?distrito_nombre=${distrito.value}`
      )
        .then((res) => res.json())
        .then((datos) => {
